...
from musicaljuggling.DLX.dlxm import DLXM
...
def solve_exact_cover_with_milp(ec_instance: ExactCoverInstance,
    ...
    p = pulp.LpProblem(name = "ExactCover", sense = pulp.const.LpMinimize)
    ...
    x = pulp.LpVariable.dict("x", list(variables), cat=pulp.const.LpBinary)
    for item in ec_instance.prim_items:
        if len(d_prim[item]) > 0:
            ...
            ## p.add_constraint(item.bounds[0] <= sum(rows_vars) <= item.bounds[1])
            ## p += item.bounds[0] <= sum(rows_vars) <= item.bounds[1] #TODO possible de double ineg ?
            p += item.bounds[0] <= pulp.lpSum(rows_vars)#TODO possible de double ineg ?
            p += pulp.lpSum(rows_vars) <= item.bounds[1]
        # elif isinstance(item, CItem):
        ...
        for item in ec_instance.prim_items:
            if len(d_prim[item]) > 0:
                if isinstance(item, WItem):
                    variables2.add((item.time, item.hand))

        o = dict()
        for time, hand in variables2:
            o[(time, hand)] = pulp.LpVariable(f"o_{time}_{hand}", cat=pulp.const.LpBinary)
        #o = pulp.LpVariable.dict("o", list(variables2), cat=pulp.const.LpBinary)
        for item in ec_instance.prim_items:
            if len(d_prim[item]) > 0:
                rows_vars = [x[i] for i in d_prim[item]]
                if isinstance(item, XItem):
                    ...
                    if item.flying_time in maximize:
                        ...
                        max_throws += pulp.lpSum(rows_vars)
                    else:
                        ## min_throws += sum(rows_vars)
                        ## min_throws_high += len(rows_vars)
                        min_throws += pulp.lpSum(rows_vars)
                        min_throws_high += len(rows_vars)
    ...
    a = ExactCoverSolution(rows=[ec_instance.rows[i]
        for i in variables if pulp.value(x[i]) == 1.0],
        params=ec_instance.params)
    #print(a)
    return a
